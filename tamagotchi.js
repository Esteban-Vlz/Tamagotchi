...
class Hambriento extends Estado {
    alimentar(tamagotchi) {
        ...
        tamagotchi.hambre -= hambreReducida;
        tamagotchi.felicidad = Math.min(100, tamagotchi.felicidad + 5);
        tamagotchi.aburrimiento = Math.min(100, tamagotchi.aburrimiento + 20);
        console.log(`${tamagotchi.nombre} está siendo alimentado. Hambre reducida en ${hambreReducida}. Hambre actual: ${tamagotchi.hambre}`);
...
class Cansado extends Estado {
    dormir(tamagotchi) {
        tamagotchi.energia = Math.min(100, tamagotchi.energia + 30); // Límite de energía a 100
        console.log(`${tamagotchi.nombre} está durmiendo. Energía: ${tamagotchi.energia}`);
        tamagotchi.hambre = Math.max(0, tamagotchi.hambre - 10);  // Baja algo el hambre al dormir
...
class Tamagotchi {
    ...
    alimentar() {
        if (this.vivo) {
            if (this.hambre === 0) {
                ...
                console.log(`El Tamagotchi ${this.nombre} está hambriento.`);
                this.felicidad = Math.max(0, this.felicidad - 20);
    ...
    dormir() {
        if (this.vivo) {
            if (this.energia <= 20) {
                ...
                setTimeout(() => this.wakeUp(), 300000); // Despierta después de 5 minutos
            } else {
    ...
    verificarEstado() {
        if (this.hambre >= 100 || this.salud <= 0 || this.felicidad <= 0) {
            console.log(`${this.nombre} ha fallecido... fin del juego.`);
            this.matar();
        }
    }
    ...
    incrementarEdad() {
        if (this.vivo) {
            this.edad += 1;
            if (this.edad >= 100) {
                console.log(`${this.nombre} ha vivido una larga vida. Fin del juego.`);
                this.matar();
            }
        }
    }
    ...
    iniciarTemporizadores() {
        setInterval(() => {
            if (this.hambre < 100) this.hambre += 1;
            if (this.energia > 0) this.energia -= 1;
            if (this.salud > 0 && this.energia <= 20) this.salud -= 0.1; // Baja la salud de forma progresiva con poca energía
            this.actualizarBarras();
        }, 60000); // Cada minuto se incrementan el hambre y se reduce la energía

    }

    actualizarBarras() {
        // Llamar a cada observador para actualizar las barras
